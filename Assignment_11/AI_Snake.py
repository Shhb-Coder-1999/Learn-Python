...
class Game(arcade.Window):
     ...
     def on_update(self, delta_time: float):
        X_pos = 0
        Y_pos = 0
        if math.sqrt((self.snake.body[0][0]-self.apple.apple.center_x)**2+(self.snake.body[0][1]-self.apple.apple.center_y)**2)<\
            math.sqrt((self.snake.body[0][0]-self.pear.pear.center_x)**2+(self.snake.body[0][1]-self.pear.pear.center_y)**2):      
            X_pos=self.apple.apple.center_x
            Y_pos=self.apple.apple.center_y
        else :
            X_pos=self.pear.pear.center_x
            Y_pos=self.pear.pear.center_y
        
        print(X_pos)
        print(Y_pos)


        if  self.snake.body[0][0]>X_pos:
            self.on_key_release(symbol=arcade.key.LEFT,modifiers=None)

        elif self.snake.body[0][0] <X_pos:
            self.on_key_release(symbol=arcade.key.RIGHT,modifiers=None)

        elif self.snake.body[0][1] < Y_pos:
            self.on_key_release(symbol=arcade.key.UP,modifiers=None)

        elif self.snake.body[0][1]>Y_pos:
            self.on_key_release(symbol=arcade.key.DOWN,modifiers=None)
